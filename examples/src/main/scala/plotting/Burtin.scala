...
"""
    ...
    val drug_color = Map[String, Color](
        ("Penicillin",   "#0d3362"  ),
        ("Streptomycin", "#c64737"  ),
        ("Neomycin",     Color.Black))

    val gram_color = Map[String, Color](
        ("positive", "#aeaeb8"),
        ("negative", "#e69584"))
    ...
    */
    ...
    val height = 800
    val inner_radius = 90
    val outer_radius = 300 - 10

    /*
    val minr = sqrt(log(.001 * 1E4))
    val maxr = sqrt(log(1000 * 1E4))
    val a = (outer_radius - inner_radius) / (minr - maxr)
    val b = inner_radius - a * maxr

    def rad(mic: Vector[Double]) = np.sqrt(np.log(mic * 1E4))*a + b

    val big_angle = 2.0 * np.pi / (len(df) + 1)
    val small_angle = big_angle / 7
    ...
    line(x+1, y+1, alpha=0, width=width, height=height, title="", tools=Pan|WheelZoom|BoxZoom|Reset|PreviewSave, x_axis_type=None, y_axis_type=None)
